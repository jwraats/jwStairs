...
{
    var frameItems = await db.Frames.Include(f => f.Leds).Where(frame => frame.SceneId == sceneId).OrderBy(o => o.OrderNr).Select(frameItem => new ApiFrame(){ OrderNr = frameItem.OrderNr, Leds = frameItem.Leds.Select(led => new ApiLed(){
            LedNr = led.LedNr,
            ColorRed = led.ColorRed,
            ColorGreen = led.ColorGreen,
            ColorBlue = led.ColorBlue,
            ColorAlpha = led.ColorAlpha
        }).ToList(), WaitTillNextFrame = frameItem.WaitTillNextFrame}).ToListAsync();
...
// !! ========= Animations ========= !!
app.MapGet("/animation/{show}", async (LedDbContext db, string show, string? color, string? blankColor, bool repeat = false) =>
{
    ...
    switch (show)
    {
        ...
        case "theatrechase":
            ...
            break;
        case "rainbow":
            animationTask = Task.Run(() => effects.Rainbow(cancellationTokenSource.Token));
            break;
        case "colorwipe":
            if(color != null)
                animationTask = Task.Run(() => effects.ColorWipe(System.Drawing.ColorTranslator.FromHtml($"#{color}")));
            break;
        case "color":
            if(color != null)
            ...
            break;
        default:
            var scene = await db.Scenes.Include(s => s.Frames).ThenInclude(f => f.Leds).FirstOrDefaultAsync(scene => scene.Name == show);
